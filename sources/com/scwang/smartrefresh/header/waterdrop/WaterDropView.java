...
public class WaterDropView extends View {
    ...
    private void makeBezierPath() {
        ...
        if (this.bottomCircle.y > this.topCircle.y + DensityUtil.dp2px(1.0f)) {
            ...
            float cos = (float) (this.topCircle.x - (this.topCircle.radius * Math.cos(angle)));
            float sin = (float) (this.topCircle.y + (this.topCircle.radius * Math.sin(angle)));
            float cos2 = (float) (this.topCircle.x + (this.topCircle.radius * Math.cos(angle)));
            float cos3 = (float) (this.bottomCircle.x - (this.bottomCircle.radius * Math.cos(angle)));
            float sin2 = (float) (this.bottomCircle.y + (this.bottomCircle.radius * Math.sin(angle)));
            float cos4 = (float) (this.bottomCircle.x + (this.bottomCircle.radius * Math.cos(angle)));
            this.mPath.moveTo(this.topCircle.x, this.topCircle.y);
            this.mPath.lineTo(cos, sin);
            this.mPath.quadTo(this.bottomCircle.x - this.bottomCircle.radius, (this.bottomCircle.y + this.topCircle.y) / 2.0f, cos3, sin2);
            this.mPath.lineTo(cos4, sin2);
            this.mPath.quadTo(this.bottomCircle.x + this.bottomCircle.radius, (this.bottomCircle.y + sin) / 2.0f, cos2, sin);
        }
        this.mPath.close();
    }
    ...
    public void updateCompleteState(float f) {
        ...
        this.topCircle.radius = f2;
        this.bottomCircle.radius = f3;
        this.bottomCircle.y = this.topCircle.y + (f * 4.0f * i);
    }
    ...
    public void updateCompleteState(int i) {
        ...
        float pow = (float) ((i2 - this.mMinCircleRadius) * (1.0d - Math.pow(100.0d, (-Math.max(0.0f, r4 - r3)) / DensityUtil.dp2px(200.0f))));
